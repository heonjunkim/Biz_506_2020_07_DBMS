...
public class OrderInput {
	...
	public boolean orderInput(OrderVO orderVO) {
		...
		System.out.printf("주문번호(%s, QUIT:중단)>> ",
				orderVO.getO_num() == null ? "새로추가" : orderVO.getO_num());
		 ...
		 * 아래쪽 orderVO.getO_num().isEmpty()는 그냥 통과 될것이다.
		 */
		if(!strNum.isEmpty()) {
		...
		while(true) {
			System.out.printf("상품코드(%s, QUIT:중단)>> ",
					orderVO.getO_pcode() == null ? "NEW" : orderVO.getO_pcode());
