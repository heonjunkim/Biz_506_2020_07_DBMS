...
--      CREATE USER 사용자 정도 이며 Schema등을
--      생성하는 명령 절차
...
-- Table 명명규칙 : 보통 tbl_ 접두사로 시작한다. snack case 형식으로 이름 지정
CREATE TABLE tbl_student (
    -- 칼럼(Field) 명명규칙 : 보통 table이름을 줄여서 접두사 시작
    --      snack case 형식으로 이름 지정
    -- 문자열 칼럼의 type
    -- CHAR, VARCHAR2
    -- CHAR : 고정길이 문자열, 칼럼에 저장되는 데이터의 길이가 모두 같은 경우에
    --      주로 사용된다.
    --      저장되는 데이터가 설정한 크가보다 작으면 남는 공간을 공백으로 채운다.
...
-- 학번이 10002, 10003, 10005,10006 인 학생의 학과를 화확과로 변경하고 싶다.
-- 여러번의 명령 수행하지 않고 변경할수 있는 방법
...
-- 데이터를 Table형식으로 보여줄때 모든 칼럼을 다 포함ㅎ여 보여달라
-- SELECTION type 으로 table 보기
SELECT * FROM tbl_student ;

-- 데이터를 Table 형식으로 보여줄때 나열된 칼럼들만 보여달라
...
SELECT st_name || '는' || st_dept || '학과 학생 입니다'
FROM tbl_student
WHERE st_dept = '음악과';
